  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-24 pb-16 text-center">
        <Tag color="blue" className="mb-6 text-sm px-3 py-1 rounded-full">
          ✨ No code required
        </Tag>
        <Title
          level={1}
          className="!text-5xl !font-extrabold !text-slate-900 !mb-6 !leading-tight"
        >
          Build any app.
          <br />
          <span className="text-blue-600">Host on your domain.</span>
        </Title>
        <Paragraph className="!text-xl !text-slate-500 max-w-2xl mx-auto !mb-10">
        ...
        <Space size="middle" wrap>
          <Link href="/register">
            <Button type="primary" size="large" icon={<ArrowRightOutlined />} className="h-12 px-8 text-base">
              Start Building Free
            </Button>
          </Link>
          <Link href="/login">
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {TEMPLATES.map((t) => (
            <div
              key={t.label}
              className="rounded-xl border-2 p-6 text-center cursor-pointer hover:shadow-md transition-all"
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="flex gap-4 p-6 rounded-xl border border-slate-100 hover:border-blue-200 hover:shadow-sm transition-all"
              >
                ...
                <div>
                  ...
                  <Text className="text-slate-500">{f.desc}</Text>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            <div
              key={s.step}
              className="text-center p-6 rounded-xl bg-white border border-slate-100"
            >
              ...
              <Text className="text-slate-400 text-sm">{s.desc}</Text>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-blue-600 py-16">
        <div className="max-w-2xl mx-auto text-center px-6">
          ...
          <Link href="/register">
            <Button
              size="large"
              className="h-12 px-10 text-base bg-white !text-blue-600 border-white hover:!bg-blue-50 font-semibold"
            >
              Create Free Account
            </Button>
          </Link>
